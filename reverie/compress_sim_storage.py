...
def compress(sim_code):
    ...
    sim_storage = base_dir / "environment" / "frontend_server" / "storage" / sim_code
    compressed_storage = (
        base_dir / "environment" / "frontend_server" / "compressed_storage" / sim_code
    )
    ...
    for i in range(max_move_count+1):
        master_move[i] = dict()
        with open(move_folder / f"{str(i)}.json") as json_file:
