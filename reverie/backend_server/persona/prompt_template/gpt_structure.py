...
Description: Wrapper functions for calling OpenAI-compatible APIs.
"""
import json
import os
import random
...
def GPT_request(prompt, gpt_parameter):
    ...
    temp_sleep()
    try:
        # Use chat endpoint to stay compatible with DashScope and OpenAI.
        model = gpt_parameter.get("engine") or gpt_parameter.get(
            "model") or default_chat_model
        # Force to Qwen if legacy GPT names are present.
        if isinstance(model, str) and (model.startswith("text-") or model.startswith("gpt-")):
            model = default_chat_model
...
def safe_generate_response(prompt,
    ...
    for i in range(repeat):
        ...
        if not curr_gpt_response or "TOKEN LIMIT EXCEEDED" in curr_gpt_response:
            continue
        if func_validate(curr_gpt_response, prompt=prompt):
            return func_clean_up(curr_gpt_response, prompt=prompt)
        if verbose:
...
def get_embedding(text, model=None):
    ...
    try:
        ...
        # Network/credential errors should not crash the sim; fall back to a
        # deterministic dummy embedding with non-zero norm.
        try:
            fallback_dim = int(os.getenv("LLM_EMBED_DIM", "1536"))
        except Exception:
            fallback_dim = 1536
        fallback = [0.0] * fallback_dim
        fallback[0] = 1.0
        print("get_embedding fallback used:", e)
        return fallback
