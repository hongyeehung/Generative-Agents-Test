...
In the `reverie/backend_server` folder (where `reverie.py` is located), edit `utils.py` to set your name in `key_owner`. API keys are read from environment variables so they are not hard-coded:

- Set `DASHSCOPE_API_KEY` (recommended) to use Aliyun DashScope/Qwen.  
- Or set `OPENAI_API_KEY` if you want to call OpenAI directly.

You can optionally override the model and endpoint by exporting `LLM_MODEL` (defaults to `qwen-max`) and `OPENAI_API_BASE`/`DASHSCOPE_API_BASE` (defaults to DashScope's compatible endpoint).
